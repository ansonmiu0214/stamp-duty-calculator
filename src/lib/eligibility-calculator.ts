...
export type Eligibility = IEligible | INotEligibleYet | INeverEligible;

const daysRequired = 183;

export function refundEligibilityForWindow(
	windowStartDate: Date,
	windowEndDate: Date,
	trips: Trip[]
): Eligibility {
	...
	const numValidDays = validTrips
		.map((trip) => computeOverlap(trip, window))
		.reduce((x, y) => x + y, 0);
	...
	if (
		validTrips.length > 0 &&
		D.daysBetween(validTrips[validTrips.length - 1].endDate, windowEndDate) >= numDaysMissing
	) {
...
export function optimalRefundEligibility(completionDate: Date, trips: Trip[]): Eligibility {
	...
	const earliestWindowStartDate = D.calculateFrom(completionDate)
		.applyDelta({ days: -364 })
		.equals();
	...
	for (const windowStartDate of D.xrange(earliestWindowStartDate, latestWindowStartDate)) {
		...
		switch (eligibility.result) {
			...
			case 'no, but possible!': {
				...
				if (
					shortestTripExtension === undefined ||
					eligibility.numDaysMissing <= shortestTripExtension.numDaysMissing
				) {
			...
			case 'no, impossible!': {
				if (
					maxValidWindow === undefined ||
					eligibility.numValidDays >= maxValidWindow.numValidDays
				) {
